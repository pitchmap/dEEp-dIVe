...
export class CreditDropField {
  ...
  spawnCredits(x: number, y: number, z: number, amount: number): void {
    if (!Number.isFinite(amount) || amount <= 0) return;
    this.items.push({ id: this.nextId, kind: 'credits', x, y, z, amount });
    this.nextId += 1;
  }

  spawnRarePart(x: number, y: number, z: number, partId: string): void {
    this.items.push({ id: this.nextId, kind: 'rarePart', x, y, z, amount: 0, partId });
    this.nextId += 1;
  }
