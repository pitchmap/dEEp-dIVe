...
> 🆕 **[17차 개발팀 소회의 접수 — M0 집행 + M1 5창 범위표]**
> `meetings/17_devteam_boss_M1_plan.md` (2026-09-21). 16차 결의를 **실행
> 범위표로 번역한 문서**이며, 피드백 개문 마감 이후 **M1의 입력은 이 범위표가
> 전부**다. 확정 사항:
> ① **M0 집행** — 3차 대회의 때 확보한 **내장그래픽 노트북 2대**에서
> `?quality=low/medium/high` × (협곡 주행·폭뢰 피격·심해 하강) fps 계측,
> 드로우 콜 65 대조, 텍스처 404 fallback 미발동 확인 →
> `docs/measurements/M0_gpu_baseline.md` 커밋 → PR #7을 **merge commit
> (`[M0]` 태그 + 측정 문서 경로)** 로 병합. 기본 품질은 **계측치가 자동 결정**
> (medium이 60fps 충족 시 medium, 미달 시 근거 기록 후 low). INT-RENDER-013
> 확정 문구는 실측 이상 없을 때 리드가 당일 기입. **일정: 실측 D0, 병합 D1.**
> 병합 후 dev tip = M1 전 창의 base
> ② **어뢰 캠 규격 충돌 해석**(개정 아님 — 후행 대회의가 선행을 대체, 침묵한
> 부분은 존치): 진입(발사 시 자동 전환)·복귀 키(Space)·조종 불가·위협 사운드
> 유지는 **16차가 정본**. 12차의 **자동 복귀 조건 3종**(① 잠수함 피격
> ② 탐지 alert 전환 ③ 어뢰 명중·소멸·사거리 종료)은 **보완 규격으로 존치**
> (16차의 '명중·빗나감 복귀'는 ③의 축약). ⚠️ **F 홀드 진입과 화면 버튼
> 진입점은 명시적 폐기 — `F` 키는 키맵에서 미배정으로 반환**한다
> ③ **M1 5창 범위표** — 보스 = **3단계 × 4패턴**(돌진 / 투사체 / 약점 개방 /
> 최종 가속), `DestroyerAIController` 확장 계보 유지, 신규 알고리즘 0.
> **소환·회전 근접은 스텁조차 금지**(부활은 지정순 — 소환 1순위).
> 창 2 `InteractionSystem`(E 2초 홀드 + 회수 중 소음, 금괴·salvage·단서·심층
> 지점 공용)은 **보스 착수 전 선행**, 파밍 보상·소나 스코프는 **병렬**,
> 침묵 항행 연결은 **완주 판정 직후 같은 스프린트**. 그래픽스 분절 애니
> 스파이크는 `feat/render` base 선행 착수 **조건부 허용**(리베이스 비용은
> 그래픽스 창 전액 부담, 병합은 어떤 경우에도 M0 후 dev 기준)
> ④ **접합면 2건** — `SonarScopeReadModel`은 **`noiseFactor` 단일 의존**
> (스코프는 침묵 항행의 존재를 몰라야 한다 → 침묵 연결 시 코드 변경 0,
> 이중 밸런스 차단). `depthChargeEnteredWater`/`depthChargeExploded`는
> **이번 스프린트에 `DepthChargeRunSystem`이 실 발행자**가 되고(현재 계약
> 선언만·발행자 0건 해소, 폭뢰 사운드 트리거도 같은 지점), 패시브 스코프
> 표시 여부는 `params.depthChargeOnPassiveScope` **초기값 false**
> (변경 권한은 기획·사운드의 튜닝표 행 추가)
> ⑤ **일정** M0 D0~D1 / `InteractionSystem` D2~D3 / 보스 코어+병렬 D2~D9 /
> **회색 상자 완주 판정 D10**(dev 통합 빌드에서 수행 — 격파 **및** 파괴
> 양쪽 경로 각 1회) / 침묵 연결 D11 / 밸런스 튜닝 D12~D13 / **M1 봉인 D14**.
> M3 앞머리(어뢰 캠·버튼 제거·프리룩)는 M1 봉인 후 별도.
> 리스크 R-M1(스파이크 실패 → B안 0.5일) · R-M2(medium 60fps 미달 → low
> 확정 + 드로우 상한 재산정, 재회의 불요) · R-M3(D10 판정 실패 → 4패턴을
> 2패턴으로 축소, **컷은 플래그 오프 — 약점 '판정' 코드는 유지하고 '패턴'
> 노출만 끈다**, 발동 권한 리드 단독)
>
> 🆕 **[16차 대회의 접수 — 다음 마일스톤 = 첫 보스 데모]**
> `meetings/16_grand_demo_roadmap.md` (2026-09-18). **A/B/C 스프린트 다음의
> 기준 문서다.** 확정 사항:
> ① 다음 공식 마일스톤 = **첫 보스 데모(M0~M4)**. 기본 품질(low/medium)은
> M0 실측이 자동 결정, 보스 MVP는 **4패턴 우선 봉인**(소환·회전 근접은 부활
> 후보 1·2순위), 단서 3개는 기존 콘텐츠 재사용
> ② 침묵 항행 규격 확정(토글 / 속도 30% / 소음 0.2배 / 산소 1.5배 / BGM 제거,
> 전부 params) — 보스 회색 상자 완주 직후 **같은 스프린트 내** 연결
> ③ 플레이테스트 수정 5건 채택(합계 7.5일):
> **관전형 어뢰 추적 캠**(조종 불가·자동 복귀·Space 즉시 복귀) /
> **인게임 화면 버튼 전면 제거 — 8차 결의 1 폐지**(사용률 2.1% 계측 근거,
> Pointer Lock·조작 안내는 시작 화면 '클릭하여 잠항 시작' 오버레이가 승계) +
> **Alt 토글 프리룩**(조준 중 진입 불가) + **Space = 만능 시점 복귀 키** /
> 금괴·난파선 **파밍 보상**(기존 단일 재화, 해역당 전투 보상의 40% 상한) /
> **E 2초 홀드 회수 + 회수 중 소음** — 금괴·단서 공용 `InteractionSystem`
> (M2 배관 겸용) / **상단 상태바 폐지 → 좌상단 다이제틱 소나 스코프**
> (패시브 방위 + 액티브 핑, 상태는 테두리색)
> ④ 편입 순서: 상호작용 **선행** / 보상·스코프 **병렬** / 카메라 2건 **후행**(M3 앞머리, M3 +1.5일)
> ⑤ **피드백 개문 마감**(CEO 발의) — 다음 창구는 M4 정식 테스트
> ⚠️ **현행 코드·문서와 충돌하는 항목**(같은 작업 단위에서 코드·문서 동시 갱신
> 필요, 12차 결의 1-⑦): 인게임 화면 전투 버튼(`src/ui/ControlsHud.ts`의
> `.hud-buttons`)은 **제거 대상**이고, `Space`는 카메라 리센터 전용에서
> **만능 시점 복귀**로 의미가 확장된다. 튜닝표 신규 5행(파밍 상한·핑 쿨다운·
> 핑 게이지 상승·회수 홀드·어뢰 캠 복귀 지연)은 기획 반영 대기.
> DECISIONS.md 반영은 리드 권한.
>
> 🆕 **[회의록 12~15 접수 — 스프린트 A/B/C 체제 확정]** PvE MVP 1차 통합
> **이후**의 계획 회의 4건이 저장소에 반영됐다. **다음 작업은 이 4건이 기준이다**:
> ① `meetings/12`(7차 대회의 개정판) — D+9 성장 루프 빌드 통과 확인 +
> **전 심도 조준·자동 부상 제거**(현행 '전 심도 조준(구 심도 전용 규칙 폐기)' 규칙 **삭제 대상**),
> 선수 조준 카메라 규격 7항(`torpedoTubeAnchor` 단일 앵커), 어뢰 캠(F 홀드,
> A~C 이후 착수), **스프린트 A(조준+성장 UI) → B(위험과 보상) → C(생존 전투)
> 순차 게이트**, 경제 데이터 확정 → UI 구현 순서 강제, **보스 진입 게이트 6조**
> ② `meetings/13`(+보완분) — 규격 검증: 게이트 충돌 해소(B만 핵심/최종 2단계),
> **2소켓 구조**(aimCameraSocket / torpedoSpawnSocket), `params/aiming.json` 신설,
> **저장 시점 5종**, **C9 신설**([COMBAT] 태그), 구매 **원자적 트랜잭션**(A5-T1~T6),
> 각도는 양수 크기 저장, `aimReturnBehavior` **스키마 제외**(reset만), B7 측정 규격
> ③ `meetings/14` — **5작업 창 체제**·범위 고정·병합 순서(리드→게임플레이→
> 그래픽스→툴링)·판정은 dev 통합 빌드에서만·A 통합 PR = B 착수 방아쇠
> ④ `meetings/15` — B·C 범위표 **조건부 사전 결의**(발효 게이트·서면 확인·
> diff 한정 개정), **병렬 슬롯**(B6·B7, 주당 20%), 파괴 정산 저장 개정,
> **상설 규칙 7조**.
> ⚠️ 아래 두 배너와 본문의 "전 심도 조준(구 심도 전용 규칙 폐기) 조준"·"조준이 수직 위치를 바꾸지 않음 (구 자동 상승 규칙 폐기)" 관련
> 서술은 12차 결의 1·⑦로 **삭제·개정 대상**이다(코드-문서 동시 갱신 원칙).
> 스프린트 A 착수 시 본문·DECISIONS·CURRENT_STATUS를 같은 작업 단위에서 갱신할 것.
>
...
7. **조준·발사:** 마우스·화면 버튼 모두 **동일 `gameplay.aim` 인스턴스**의
   beginAim/endAim/fireTorpedo만 호출. 전 심도 조준(구 심도 전용 규칙 폐기), 자동 락온 없음,
   한 입력 한 발, 잔탄·재장전 공유.
...
| 우클릭 홀드 / 좌클릭 | 조준(전 심도 조준(구 심도 전용 규칙 폐기)) / 어뢰 발사 |
