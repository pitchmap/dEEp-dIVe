...
- **진행 중:** 없음
- **다음 작업:** D6 — 코어 전투 루프 착수 (화물선·임시 탐지·구축함 AI 2상태·어뢰·폭뢰 — AimSystem 구현 포함), INT-CORE-003 적용분(각 파트) feat→dev 병합 리뷰, CanyonLayout 데이터 모듈 위치 확정
- **차단 문제:** 없음
- **변경된 계약:** INT-CORE-003 — `SubmarinePoseSource`·`CargoShipStateSource`·`torpedoHit`·`contracts/layout.ts` 추가 (리드 승인, INTERFACES.md §1·§2b·§3 동시 갱신). 이전: INT-CORE-002(AimSystem·aimModeChanged·propellerIdleSpinRatio), INT-GAME-001
- **통합 주의사항:**
