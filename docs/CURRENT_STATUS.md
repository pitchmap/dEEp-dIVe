...
- **완료(추가): [M1·M2 Runtime Closure 그래픽스 — INT-CORE-022 이행]** (base = dev `e01ffc5` 일반 merge 수신, 충돌 0) ① **world 신설 2파일(승인 범위만)**: `src/world/bossPlacement.ts` — `BOSS_PLACEMENT`(entityId 7000 — 화물선 소수/경비 8000+/salvage 9000+와 무충돌 대역, 스폰 x=centerAt(52)≈5.03·y=-4 중층·z=52·heading 0=남향 대면)·`BOSS_WEAK_POINT_PLACEMENT`(id 7001, 월드 절대 좌표 — syncTo 추종 전 초기값)·`BOSS_ZONE`(`boss-zone-abyss`, x −8~17 · z 44~60 최북단 수로 — salvage-3(z=42) 구역 밖이라 일반 파밍 동선 비간섭). 판정·타이머·상태·밸런스 수치 0 / `src/world/bossCluePlacements.ts` — 단서 interactable 정확히 3개(`wreck-datacore`(-4.93,-18.8,-38 — salvage-1 곁 8.1m)·`survey-probe`(4.25,-18.8,4)·`patrol-blackbox`(-3.03,-18.8,-52)) + `CLUE_ID_BY_INTERACTABLE`(canonical 3종 정확 커버 — targetId≠clueId·문자열 파생 없음). 전부 해저 착저 -18.8(잠항 하한 -19.1 위)·상호 최소 14.1m(회수 반경 계약 상한 12m 초과)·벽/기둥 여유 8m+ ② **단서 시각 표식**(`ClueMarkerVisuals` — 렌더 소유): 배치 파일 읽기 전용 + 점멸 표시등, 회수 반영은 `interactionCollected` targetId 통지만(판정 0) ③ **BossCoreView 소비 마감**: 공급자가 처음 비-null 뷰를 준 프레임에 production 보스 시각물 지연 장착(`mountProductionBossSpike` — 배치 파일 스폰 포즈 읽기 전용, autoDemo·검수 키 없음, 자체 순찰 발명 없음), `applyCoreView` 5상태 매핑은 기존 유지. 스폰 전 공급 게이트 권고 = INT-RENDER-016 ④ **공식 `bossHit {kind}` 소비**: attachEventBus에 구독 추가 — weakPoint→`notifyWeakpointHit`/hull→`notifyNormalHit` (명중 1건=반응 1회, 재계산 0, bossHit→bossDefeated 순서는 버스 수신 순서 그대로). 검수 키 [6]/[7]은 `?bossSpike=1` fixture 격리 유지 ⑤ **SonarBlipKind 7종 표현**: 받은 kind로만 분기(재추측 0) — ship 원점/torpedo 방위 정렬 대시/depthCharge X+점멸/goldCache 마름모/salvage 정사각/clue 삼각+확장 펄스/deepSite 이중 원, 거리 미상 호는 kind별 대시 패턴 — **전 품질 동일 2D 도형이라 low에서도 색 외 구분**(가짜 문자·네온 0). 연출값 5종은 renderVisualParams sonarScope 소유. fixture `?sonardemo=1`에 탐색 4종 표본(핑 노출 중에만 — 공급 규칙 동일) 추가 ⑥ **입력 도움말**: `controlsConfig`에 'F (홀드) 상호작용·회수' 추가(E=상승 무변경 — 인계표 §9). **액티브 핑 키는 인계표 미확정 — 미표시 유지 + blocker 보고** ⑦ **DetectionHud 이번 PR 미제거**(§7 순서: production 소나 wired 확인(Exit 14) 후 후속 그래픽스 PR에서 제거 + verify:hud 갱신)
