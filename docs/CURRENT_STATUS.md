...
- **차단 문제:** 없음. 단 잠수함 포즈 연결·Game 임포트 정리는 INTEGRATION_NOTES #002 리드 승인 대기 (미승인 상태에서도 빌드·렌더 정상 — 포즈 미주입 시 원점 정지)
- **변경된 계약:** 없음 (`src/contracts/*` 미수정 — #002는 core 조립부 정리 요청)
- **통합 주의사항:** `BootstrapScene.ts`는 `CanyonScene` 별칭 재수출로 교체됨(공통 보호 파일 미수정 우회 — #002 승인 후 삭제). 카메라 입력은 게임플레이가 `cameraRig.rotate()/recenter()` 호출. X-ray 선체가 depthWrite:false이므로 이후 반투명 오브젝트(파문 등)와 renderOrder 조율 필요. 실시간 그림자·반사 금지 유지 (성능 예산 §12)
