...
- **사용자 로컬 production UX 검수 통과** (13항목 — 성능 패널·F 프롬프트·원형
  진행·취소 초기화·수집·marker 제거·진행도·clue/salvage 구분·보스 구역
  잠김/진입 가능/진입 피드백). 상세: `docs/measurements/LOCAL_UX_VERIFICATION.md`.
...
- 로컬 플레이테스트 blocker 3건 조치 (판정 무변경, HUD·배치만):
  ...
  - L-3 `ExplorationHud` 신설 — 단서 N/M(BossProgressStore 단일 정본),
    ...
    진입·교전 배너(edge 1회).
- production 실측 41/41: 단서 `survey-probe` 실제 F 홀드 회수 1/3 완주,
  링 ↔ 정본 `progress` 최대 오차 0.0000, marker 3→2 제거·재접속 후 재표시 0,
  중복 방지·clue credits 0·저장 왕복 확인. F 프롬프트 노출 상태에서
  1920×1080 · 2552×1388 레이아웃 겹침 0건. 결정적 HUD 검증 58/58.
...
- DetectionHud 제거 — 그래픽스 소유 별도 후속 (`DETECTION_HUD_REMOVED=false`).
- 최종 M1·M2 Exit gate — 보스 전투·reset·두 번째 sortie 검수 전까지 false 유지.
...
- **통합 주의사항:**
  ...
  - **M1·M2 Closure Blocker 원인 분류·종료 경로 확정 (INT-CORE-024 / M-15~M-17 — base dev `a5bdb8f`):**
    ...
    - **EC12 원인 = production pointer lock 미해제**(M-17, `exitPointerLock` 호출 0건) — 성장 사이클·EC13·EC17의 **선행 차단 요인**. 소유: 그래픽스(ControlsHud)
...
- **진행 중:** 없음
- **다음 작업:** **INT-GAME-016 공식 params 행·조립 배선 수신 후 M2 2~5단계 wired 전환**(회수 반경·소음 기여·전투 보상 평균·패시브 번짐 폭·`params/boss.json` 3종은 아직 공식 수치가 없어 기획 결정 필요), 회수·핑 **키 바인딩 결정**(`E` 상승 병행 충돌 / 핑 키 미배정 — 17차 결의 2로 `F` 반환됨), **6단계 침묵 항행**(회색 상자 완주 판정 D10 이후 착수 — 17차 결의 3 순서), INT-GAME-012 조립 배선(스폰 위치 전략·식별 소스·다중 선박 렌더 소스) + 공식 수치 4종 요청, INT-GAME-011 조립 3줄 배선, INT-GAME-004·006 리드 결정 후 잔여 provisional 이관(심도 구간 경계·비율·어뢰 사거리), 격침 보상 어뢰 +1 배선(§5.9 — torpedoHit 구독)
...
- **통합 주의사항:** 좌표 규약 — **잠수함 로컬 -Z가 선수, +Z가 선미** (`src/core/conventions.ts`만 참조). heading은 Y축 요(yaw), heading 0 선수 = 월드 -Z, 렌더는 `mesh.rotation.y = headingRadians` 그대로. 속도 소비 규칙(INT-CORE-003): 프로펠러 등 부호가 필요하면 `poseSource.forwardSpeedMetersPerSecond`(+전진/−후진), 소음 산출 등 크기만 필요하면 계약 `player.speed`(비부호). 렌더 잠수함 Y는 `poseSource.positionY`. **화물선 상태는 `gameplay.cargoShipState`(계약 CargoShipStateSource)** — composition root가 CargoShipVisual에 주입, `hit`/`sinkProgress`(시간축 게임플레이 소유)/`removed`를 매핑만 할 것, 명중 연출·오디오는 `torpedoHit` 구독. 심도 초기 구간은 y=0 → `cruise`(초기 이벤트 없음). **충돌체 집합은 `gameplay.collision.colliders`(읽기 전용) 공유** — 은신 시야 차폐(D10~12)는 이 집합을 재사용할 것(별도 집합 금지). **협곡 배치의 유일 소스는 `src/world/startingCanyonLayout.ts`** — 렌더·충돌 모두 `CanyonLayout.blocks` 소비(미러 소멸), 레벨 교체 = 새 레이아웃을 `GameplaySystems` 생성자에 주입(또는 데이터 모듈 교체), 소비 중 레이아웃은 `gameplay.layout`으로 확인. **전투 입력은 반드시 `gameplay.aim`(계약 AimSystem) 하나로** — HUD 조준·발사 버튼은 composition root에서 `aim.beginAim()/endAim()/fireTorpedo()`를 호출(별도 전투 시스템 금지), UI는 `torpedo.remaining`·`torpedo.reloadRemainingSeconds`·`aim.aiming` 폴링 + `aimModeChanged` 구독. 리드샷 보조선 = `targets.list`(위치·속도) + `torpedo.torpedoSpeedMetersPerSecond` + `player` 포즈로 계산. 렌더 어뢰 항적은 `torpedo.torpedoes`(읽기 전용) 폴링. 화물선 시스템은 `targets.register()`로 표적 등록(콜백 `onTorpedoHit`는 어뢰 1발당 1회 보장)
  - **M2 1단계 — InteractionSystem (공유 회수 절차). M0~M2 지시 착수분**
    - `systems/interaction/InteractionSystem`: 금괴·salvage·단서·심층 탐사 지점이 **공유하는 단일 시스템**(대상 타입별 시스템 0 — 타입은 `kind` 태그일 뿐 분기 로직이 아니다)
    - 근접 판정 · E 홀드 · 진행률 · **취소 3종**(거리 이탈·대상 제거·입력 해제) · 완료 통지 **대상당 1회** · 같은 대상 중복 회수 불가 · UI read model(접근 가능·후보·진행률·이미 회수함·unwired)
  ...
  - **M2 2~5단계 — 17차 결의 3 창 2 범위 (단서·약점·파밍·스코프)**
    - **2단계 단서 획득** `systems/progress/CluePickupProgress`: 회수 완료 통지를 소비해 단서 진행만 소유한다. 고유 id · 중복 획득 불가 · **출항을 가로질러 누적**(재출항 초기화 대상 아님) · `restoreCollected` 복원 · 획득원 3종은 주입 정의(난파선 salvage / 적대 고가치 수송선 / 경비 심층 탐사 지점). **필요 개수는 정의 개수에서 파생**한다 — 코드에 3이 없다. **보스 구역 개방 게이트는 넣지 않았다**(17차 창 1 = 리드 소유) — `clueProgressSource`가 읽기 단면
    - **3단계 약점 판정** `BossWeakPointTarget`: 임시 배율(`provisionalBossWeakPointConfig` — 반경 6·배율 2.0/0.25)을 **삭제**하고 params 주입으로 이관. 개방/닫힘 **구분은 params 없이도 성립**하고(개방 여부는 포트가 소유하는 사실) 미확정인 것은 배율뿐이므로, unwired면 판정 반경 0·피해 0. 액티브 핑이 약점 개방 상태를 읽어 가는 접합면 제공
    - **4단계 파밍 보상** `systems/economy/SectorFarmingRewards`: 기존 단일 재화(`RunEconomy`)에만 적립 — **신규 화폐 0**. 해역당 상한 = 전투 보상 평균 × 비율(16차 결의 2-3·튜닝표) 강제, 초과분은 지급되지 않고 `forfeited`로 관측된다. **상한 미주입이면 지급 자체를 하지 않는다** — 무제한 지급은 결의가 명시적으로 거부한 방향이라 '미확정 = 무제한'으로 해석하지 않았다. 단서·심층 지점은 재화 대상이 아니다
    - **5단계 소나 스코프** `systems/sonar/SonarScopeSystem`(= 17차가 이름 붙인 `SonarScopeReadModel` 계약): 패시브 = **소음원만**·방위 중심·거리 `null`·번짐은 내 소음 계수에 비례 / 액티브 핑 = 보상·단서·지형·보스 정확 표시(거리 포함·번짐 0)·표시 시간·쿨다운. **핑의 대가는 기존 탐지 게이지 정본**에 적용된다(`SubmarineDetectionSystem.raiseGauge` — 별도 노출 정본 0). 낙하 폭뢰 패시브 표시는 params 소유·초기값 false(17차 결의 4). 방위는 축 규약 함수(`bowDirectionXZ`)에서만 얻는다
    - **17차 결의 4 준수** — 스코프는 `silentRunning` 불리언을 **입력으로 받지 않는다**. 유일한 입력은 탐지 시스템이 이미 계산한 `effectiveNoiseFactor`(침묵 배율 기적용)다. 6단계에서 침묵 항행을 연결하면 스코프는 **코드 변경 0**으로 선명해진다(검증기가 `silent` 계열 API 부재를 기계 확인)
    - **수치 전량 주입 유지** — 16·17차가 확정한 값(홀드 2.0초 / 핑 3초·30%·25초 / 파밍 상한 40% / 폭뢰 표시 false)도 **코드에 넣지 않았다**. `params/`는 기획 소유이고 `contracts`·`config`·`core`는 공통 보호이므로 공식 행과 조립 배선을 요청했다 — INT-GAME-016
...
- **완료(추가): [M0~M2 그래픽스]** ① **M0 GPU 실측 준비**: `docs/measurements/M0_gpu_baseline.md` — 측정 절차(3품질×3시나리오)·실기기 표(**미실측 공란 — 수치 미작성 원칙**)·정적 예산·swiftshader 참고치(GPU 아님 명시)·보스 전장 밀도 기준선(§E — M1 레이아웃 대기). **실기기 실측은 이 환경(원격 컨테이너, swiftshader 전용)에서 불가 — 빌드·툴 담당과 목표 노트북 2대 확보 시 수행** ② **소나 스코프**(`SonarScope.ts` — M2 최소 시각 소비자): 좌상단 원형 다이제틱 계기(16차 결의 — 화면 높이 18.0% 실측), 기존 정본만 소비 — DetectionHudView(테두리 황·unwired 빗금+'계기 미연결')·TrackingStateSource(attack→적)·noiseChanged(자기 소음 그레인·링)·ShipWorldSource(패시브 소음원 방위·번짐 — 세력 미표시, B2 조기 노출 방지). 액티브 핑은 `SonarPingSource` 미주입=미표시(INT-RENDER-014). 캔버스 15Hz 상한·WebGL 드로우 0. BASE에서 숨김 ③ **보스 5상태 표현**(M1 최소 시각 소비자): notifyWeakpointHit(백-주황 플래시+스케일 펄스)/notifyNormalHit(짧은 회청 플래시)/단계 전환(색+**전신 진폭 서지+백색 플래시** — low 품질 색 단독 의존 없음)/약점 활성·비활성 — 전부 통지 기반(판정 계산 0), `?bossSpike=1` 검수 키 6/7 ④ 분절 스파이크는 M0 tip에서 정상 유영 재확인(4인 위협감 판정 대기·B안 전환 준비 유지)
- **완료(추가): [정본 계약 소비 전환 — PR #10 수신]** ① **SonarScope 재작성**: `contracts/sonar.ts` `SonarScopeReadModel` **하나만** 소비 — 구 소스(월드 좌표·ShipWorldSource·DetectionHudView·TrackingStateSource·noiseChanged 구독) 전부 제거, blips(bearing·spread·distance·kind)·noiseFactor·ringState(safe청록/searching황/detected적)·activePing 상태만 표시. 공급자 미주입/unwired = 빗금+'계기 미연결'(production 기본 — 실측 확인). 폭뢰 필터·번짐 계산 렌더 재판단 0. fixture `?sonardemo=1`(배지) ② **보스 시각 정본 소비**: `BossCoreView` 폴링(`attachBossViewSource`) + 이벤트 3종(bossPhaseChanged/bossWeakPointChanged/bossDefeated) 구독 — 단계 3종 색+전신 서지, 약점 활성/비활성, **예고 4종**(ram 웅크림/projectile 턱 반복 개방/weakPointOpen 약점 프리펄스/phaseShift 명멸+진폭 감쇠 — 색 단독 의존 없음), **최종 가속**(3단계 유영 주기 ×1.35 시각 피드백), 격파 시 유영 정지·감광. 약점/일반 피격 구분 통지는 계약 부재(INT-RENDER-014 잔여 — notify API 준비됨) ③ Game.ts의 구 소나 배선 2줄 제거(공급자 배선 지점 주석으로 대체)
- **완료(추가): [M0 문서 단일 정본 결합]** `docs/measurements/M0_gpu_baseline.md`의 **빌드·툴 판본과 그래픽스 판본을 PR #8에서 단일 정본으로 결합**했다(중복 정본 2본 상태 해소 — 한쪽 삭제 없이 양쪽 고유 내용 전부 보존). 구성: **파트 A(그래픽스)** 재현 규격·측정 시나리오 3종·정적 렌더 예산(텍스처/파티클/광원)·품질 3단계 시각 차이표·시각 품질 확인 절차·드로우 콜 기준선 의미·보스 전장 밀도 기준·실기기 표(공란=미실측)+판정 기준 / **파트 B(빌드·툴)** 기기·GPU·브라우저 환경 식별·software renderer 및 SwiftShader 자동 판별·실제 GPU 증적 판정 규칙·Windows/Linux 측정 주의·자동 집계 절차·컨테이너 참조 측정치 / **파트 C** 미실시 항목·후속. 원자료 정본 경로는 **`docs/measurements/m0-measurement.json` 하나**로 유지(복제 금지). 명시 상태 유지: SwiftShader = 참고치(GPU 증적 아님) · 실기기 실측 미완료 · 미측정값은 null/미실측 · `defaultQualityDecision=null` · 드로우 콜 65 = 정적 기준선 · INT-RENDER-013 미승인 · M0 미종결
- **완료(추가): [M1·M2 Runtime Closure 그래픽스 — INT-CORE-022 이행]** (base = dev `e01ffc5` 일반 merge 수신, 충돌 0) ① **world 신설 2파일(승인 범위만)**: `src/world/bossPlacement.ts` — `BOSS_PLACEMENT`(entityId 7000 — 화물선 소수/경비 8000+/salvage 9000+와 무충돌 대역, 스폰 x=centerAt(52)≈5.03·y=-4 중층·z=52·heading 0=남향 대면)·`BOSS_WEAK_POINT_PLACEMENT`(id 7001, 월드 절대 좌표 — syncTo 추종 전 초기값)·`BOSS_ZONE`(`boss-zone-abyss`, x −8~17 · z 44~60 최북단 수로 — salvage-3(z=42) 구역 밖이라 일반 파밍 동선 비간섭). 판정·타이머·상태·밸런스 수치 0 / `src/world/bossCluePlacements.ts` — 단서 interactable 정확히 3개(`wreck-datacore`(-4.93,-18.8,-38 — salvage-1 곁 8.1m)·`survey-probe`(4.25,-18.8,4)·`patrol-blackbox`(-3.03,-18.8,-52)) + `CLUE_ID_BY_INTERACTABLE`(canonical 3종 정확 커버 — targetId≠clueId·문자열 파생 없음). 전부 해저 착저 -18.8(잠항 하한 -19.1 위)·상호 최소 14.1m(회수 반경 계약 상한 12m 초과)·벽/기둥 여유 8m+ ② **단서 시각 표식**(`ClueMarkerVisuals` — 렌더 소유): 배치 파일 읽기 전용 + 점멸 표시등, 회수 반영은 `interactionCollected` targetId 통지만(판정 0) ③ **BossCoreView 소비 마감**: 공급자가 처음 비-null 뷰를 준 프레임에 production 보스 시각물 지연 장착(`mountProductionBossSpike` — 배치 파일 스폰 포즈 읽기 전용, autoDemo·검수 키 없음, 자체 순찰 발명 없음), `applyCoreView` 5상태 매핑은 기존 유지. 스폰 전 공급 게이트 권고 = INT-RENDER-016 ④ **공식 `bossHit {kind}` 소비**: attachEventBus에 구독 추가 — weakPoint→`notifyWeakpointHit`/hull→`notifyNormalHit` (명중 1건=반응 1회, 재계산 0, bossHit→bossDefeated 순서는 버스 수신 순서 그대로). 검수 키 [6]/[7]은 `?bossSpike=1` fixture 격리 유지 ⑤ **SonarBlipKind 7종 표현**: 받은 kind로만 분기(재추측 0) — ship 원점/torpedo 방위 정렬 대시/depthCharge X+점멸/goldCache 마름모/salvage 정사각/clue 삼각+확장 펄스/deepSite 이중 원, 거리 미상 호는 kind별 대시 패턴 — **전 품질 동일 2D 도형이라 low에서도 색 외 구분**(가짜 문자·네온 0). 연출값 5종은 renderVisualParams sonarScope 소유. fixture `?sonardemo=1`에 탐색 4종 표본(핑 노출 중에만 — 공급 규칙 동일) 추가 ⑥ **입력 도움말**: `controlsConfig`에 'F (홀드) 상호작용·회수' 추가(E=상승 무변경 — 인계표 §9). 액티브 핑 키는 교차 감사에서 **Q로 확정** — 'Q 액티브 소나 핑' 표기 추가(owner gameplay, 쿨다운 수치 미탑재) ⑦ **DetectionHud 이번 PR 미제거**(§7 순서: production 소나 wired 확인(Exit 14) 후 후속 그래픽스 PR에서 제거 + verify:hud 갱신) ⑧ **교차 감사 보완(F-3·F-4)**: BossCoreView null 복귀 시 production 보스 시각물 해제(INT-RENDER-016 최종 결정 이행 — dispose+scene 제거+참조 null, fixture 비대상, 재장착 = 새 인스턴스, 검수 `?bossviewdemo=1`) + `CanyonScene.markCluesCollected(targetIds)` save 복원 단서 표식 동기화 API(실시간 경로와 동일 내부 제거 재사용, idempotent·미지 ID 안전, 매핑 역조회는 통합 관리자 소유)
- **마지막 업데이트:** **origin/dev(097f365 — PR #14 게임플레이 E/F/Q·provider · #15 params·loader·RC verifier · #17 승인 초기값) 일반 merge 수신, 충돌 0** + C1 정본 어휘 정합(배치 필드 `interactableId`→`targetId` — 공유 계약 `InteractionCollectedEvent.targetId` 어휘, 좌표·매핑 무변경) → verify:runtime-closure **fail 0 · C1 pass · Q1 pass · blockedByNullParam 0 · wiring 15 unwired 유지**, enforce-exit 사유는 wiring·manual·Exit 계열만. 이전: **M1·M2 Runtime Closure 그래픽스 이행** (feat/render — 위 완료(추가) 참조). 자동: typecheck·build·size 7.1%·scope(보스 1/1)·gameplay 291/291·meta 164/164·tooling 36/36·hud 34/34·sprint-a/b/c 통과. 이전: **origin/dev(9252d58 — PR #11 빌드·툴 · PR #12 게임플레이 foundation) 병합 + M0 문서 단일 정본 결합** (feat/render). 코드 충돌 0건(문서 2건만 — 양쪽 기록 보존). 그래픽스 산출물 무변경: 보스 시각 소비자(3단계·약점·예고·최종 가속)·SonarScopeReadModel 소비(월드 좌표 참조 0)·unwired 안전 표시·품질 3단계. 이전: **origin/dev(7eb8d5c — C 런타임 마감) 병합 + 통합 재검증 완료** (feat/render `a542730`). 자동: gameplay 242/242·meta 128/128·tooling 26/26·hud 34/34·sprint-a/b·sprint-c 23/23(C9 17필드 확정). 브라우저 생존 루프 실측 완주: 뇌격→경비 스폰→발각→목표 심도 폭뢰 기폭(투하 11.98→기폭 목표 = 플레이어 심도 11.0)→near 침수(첫 severity 0.10 = 승인값)→침수 잠식 파괴(120→0)→실패 화면 confirm→BASE→재출항 reset(선체/침수/salvage/탐지 전부 초기화). 4개 심도 탐지 보정·품질 3단계·텍스처 404 fallback·콘솔 오류 0. INT-RENDER-013 조건부 승인 검증 조건 이행 — 상세는 INTEGRATION_NOTES INT-RENDER-013 표
