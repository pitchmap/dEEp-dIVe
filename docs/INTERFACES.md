...
| `SonarScopeReadModel` (`contracts/sonar.ts`) | 소나 스코프 표시 단면 — unwired 자세·`noiseFactor` 단일 자기 상태(침묵 boolean 금지 — 17차 결의 4)·blips(canonical targetId·kind·방위·번짐·거리\|null·fromActivePing — **월드 좌표 금지**)·핑 타이머 2종·pingReady·ringState(`DetectionStage` 재사용). `depthChargeOnPassiveScope` 필터는 게임플레이 공급 시점 적용 — 렌더는 받은 blips 가감 금지. 공급자 `SonarScopeSystem`·소비자 `SonarScope` 존재 — production 배선은 통합 배선표(HANDOFF §5) (M-6, INT-RENDER-014) | 리드(계약) / 게임플레이(공급) / 렌더(표시) |
| `SonarBlipKind` 확장 (M-12) | 전투 3종(`ship\|torpedo\|depthCharge`) + 탐색 4종(= `InteractionTargetKind` 재사용). 탐색 kind는 **액티브 핑 노출 중에만** 공급(패시브 비노출 — 미식별 선노출 금지). 지형은 blip 아님 — 렌더가 협곡 레이아웃 단일 소스로 배경층 직접 묘화. 그래픽스는 kind 재추측 금지 (INT-CORE-022) | 리드(계약) / 게임플레이(공급 규칙) / 렌더(표시) |
| `bossHit` (`events.ts`) | 보스 피격 시각 통지 — 명중 판정 확정 1건당 1회, 약점/일반 분류·배율 적용 **후** 발행(재배율 금지), payload는 `{kind}` 최소(피해량·위치·공격자 비탑재 — 렌더는 크기 비계산). bossHit → bossDefeated 순서. 판정·피해에 영향 0 | 게임플레이(발행 — onHit 지점) / 렌더·오디오(소비) |
| `bossWeakPointChanged` 발행자 개정 (M-13) | 발행 정본 = **리드 보스 코어**(상태 전이 시에만 1회 — 개방 상태 정본이 코어 phasePort이므로). 격파 시 false → bossDefeated 순서(meta 검증 고정). 명중 '판정'은 여전히 게임플레이 소유(bossHit) | 리드(발행) / 렌더·UI(소비) |
| `ClueIdByInteractableId` (`contracts/meta.ts`) | 월드 interactable ID → canonical clueId 매핑 계약 — 작성 정본 `world/bossCluePlacements.ts`(그래픽스+기획), 검증 툴링(clueIds 대조), 주입 조립부, 최종 방어 스토어 unknownClue. 매핑 없는 clue = 발행 0 (M-9) | 리드(계약) / 그래픽스·기획(데이터) / 툴링(검증) / 조립부(주입) |
| `NullableTunable`·보스 승인 대기 4필드 (M-7) | `movement.*` 2종·`ram.contactDamage`·`weakPointOpen.hitRadiusMeters` — value null = 해당 축 unwired(이동 0·돌진 무피해·약점 명중 불가). 범위·관계 제약(평상시 ≤ 돌진)은 로더 강제. 수치는 기획 승인 후 툴링 입력 | 리드(계약·로더) / 기획(수치) / 게임플레이(소비) |
